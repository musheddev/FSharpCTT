...
let const_tp_clo tp =
  Closure (SyntaxData.TpVar 0, {tpenv = [tp]; conenv = []})

let const_tm_clo con =
  Closure (SyntaxData.Var 0, {tpenv = []; conenv = [con]})
...
let un_lam con =
  (* y, x |= y(x) *)
  Closure (SyntaxData.Ap (SyntaxData.Var 1, SyntaxData.Var 0), {tpenv = []; conenv = [con]})

let compose f g =
  Lam (Anon, Closure (SyntaxData.Ap (SyntaxData.Var 2, SyntaxData.Ap (SyntaxData.Var 1, SyntaxData.Var 0)), {tpenv = []; conenv = [g;f]}))

let apply_to x =
  Closure(SyntaxData.Ap (SyntaxData.Var 0, SyntaxData.Var 1), {tpenv = []; conenv = [x]})

let fst = Lam (Anon, Closure(SyntaxData.Fst (SyntaxData.Var 0), {tpenv = []; conenv = []}))
let snd = Lam (Anon, Closure(SyntaxData.Snd (SyntaxData.Var 0), {tpenv = []; conenv = []}))

let proj lbl = Lam (Anon, Closure(SyntaxData.Proj (SyntaxData.Var 0, lbl), {tpenv = []; conenv = []}))
let el_out = Lam (Anon, Closure(SyntaxData.ElOut (SyntaxData.Var 0), {tpenv = []; conenv = []}))
