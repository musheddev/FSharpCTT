...
module CmpM =
  ...
  let read_cof_thy : Reader<CmpL,disj_thy> = monad' {
      let! x = Reader.ask
      return x.cof_thy
  }


  let test_sequent cx phi : Reader<CmpL,_> = monad' {
      let! x = Reader.ask
      return Disj.test_sequent x.cof_thy cx phi
  }
  ...
  let abort_if_inconsistent =
    fun abort m ->
      fun st ->
        match Disj.consistency st.cof_thy with
        | true -> m st
        | false -> abort st
