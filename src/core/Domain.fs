...
let const_tp_clo tp =
  Closure (SyntaxData.TpVar 0, {tpenv = Snoc (Emp, tp); conenv = Emp})

let const_tm_clo con =
  Closure (SyntaxData.Var 0, {tpenv = Emp; conenv = Snoc (Emp, con)})
...
let un_lam con =
  (* y, x |= y(x) *)
  Closure (SyntaxData.Ap (SyntaxData.Var 1, SyntaxData.Var 0), {tpenv = Emp; conenv = Snoc (Emp, con)})

let compose f g =
  Lam (Anon, Closure (SyntaxData.Ap (SyntaxData.Var 2, SyntaxData.Ap (SyntaxData.Var 1, SyntaxData.Var 0)), {tpenv = Emp; conenv = Snoc (Snoc (Emp, f), g)}))

let apply_to x =
  Closure(SyntaxData.Ap (SyntaxData.Var 0, SyntaxData.Var 1), {tpenv = Emp; conenv = Snoc (Emp, x)})

let fst = Lam (Anon, Closure(SyntaxData.Fst (SyntaxData.Var 0), {tpenv = Emp; conenv = Emp}))
let snd = Lam (Anon, Closure(SyntaxData.Snd (SyntaxData.Var 0), {tpenv = Emp; conenv = Emp}))

let proj lbl = Lam (Anon, Closure(SyntaxData.Proj (SyntaxData.Var 0, lbl), {tpenv = Emp; conenv = Emp}))
let el_out = Lam (Anon, Closure(SyntaxData.ElOut (SyntaxData.Var 0), {tpenv = Emp; conenv = Emp}))
...
//      SyntaxData.dump_tp tp
//      (pp_list_group ~left:pp_lsq ~right:pp_rsq ~sep pp_tp) (Bwd.Bwd.to_list tpenv)
//      (pp_list_group ~left:pp_lsq ~right:pp_rsq ~sep pp_con) (Bwd.Bwd.to_list conenv)
//
//and pp_sign_clo : (SyntaxData.sign clo) Pp.printer =
//  let sep fmt () = fprintf fmt "," in
//  fun fmt (Closure(sign, {tpenv; conenv})) ->
//    fprintf fmt "tpclo[%a ; [%a ; %a]]"
//      SyntaxData.dump_sign sign
//      (pp_list_group ~left:pp_lsq ~right:pp_rsq ~sep pp_tp) (Bwd.Bwd.to_list tpenv)
//      (pp_list_group ~left:pp_lsq ~right:pp_rsq ~sep pp_con) (Bwd.Bwd.to_list conenv)
//
