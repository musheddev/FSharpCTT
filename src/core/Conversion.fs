...
struct
  ...
  let pp : t Pp.printer =
    ...
    function
    | ExpectedDimEq (r,s) ->
      fprintf fmt "Expected %a = %a : dim" D.pp_dim r D.pp_dim s
    | ExpectedSequentTrue (phis, psi) ->
      fprintf fmt "Expected @[%a |- @[%a@]@] true" D.pp_cof (Cof.meet phis) D.pp_cof psi
    | ExpectedTypeEq (tp0, tp1) ->
      fprintf fmt "Expected %a = %a type" D.pp_tp tp0 D.pp_tp tp1
    | ExpectedConEq (tp, con0, con1) ->
      fprintf fmt "Expected %a = %a : %a" D.pp_con con0 D.pp_con con1 D.pp_tp tp
    | ExpectedFrmEq (frm0, frm1) ->
      fprintf fmt "Expected %a = %a" D.pp_frame frm0 D.pp_frame frm1
    | ExpectedSignEq (sign0, sign1) ->
      fprintf fmt "Expected %a = %a sig" D.pp_sign sign0 D.pp_sign sign1
    | SpineLengthMismatch (sp0, sp1) ->
      fprintf fmt "Spine length mismatch between %a and %a" D.pp_spine sp0 D.pp_spine sp1
    | HeadMismatch (hd0, hd1) ->
      fprintf fmt "Head mismatch between %a and %a" D.pp_hd hd0 D.pp_hd hd1
end
...
and equate_con tp con0 con1 =
  ...
  | _ ->
    eprintf "failed: %a, %a@." D.pp_con con0 D.pp_con con1;
